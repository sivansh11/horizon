...
node_t new_node(uint32_t first_primitive_index, uint32_t primitive_count) {
    node_t node{};
    horizon_assert(!node.aabb.is_valid(), "node should be defaulted to not valid");
    node.is_leaf = true;
...
float cost_of_node(const bvh_t& bvh, uint32_t node_index, const options_t& options) {
    const node_t& node = bvh.nodes[node_index];
    if (node.is_leaf) return options.o_primitive_intersection_cost * node.primitive_count;
    float cost = 0;
...
void update_node_bounds(bvh_t& bvh, uint32_t node_index, aabb_t *aabbs) {
    ...
    if (node.is_leaf) {
        for (uint32_t i = 0; i < node.primitive_count; i++) {
            node.aabb.grow(aabbs[bvh.primitive_indices[node.as.leaf.first_primitive_index + i]]);
        }
    } else {
        node.aabb = {};
...
split_t find_best_split(const bvh_t& bvh, uint32_t node_index, aabb_t *aabbs, vec3 *centers, const options_t& options) {
    const node_t& node = bvh.nodes[node_index];
    ...
    switch (options.o_object_split_search_type) {
        ...
        case object_split_search_type_t::e_binned_sah: {
            aabb_t split_bounds{};
            for (uint32_t i = 0; i < node.primitive_count; i++) split_bounds.grow(centers[bvh.primitive_indices[node.as.leaf.first_primitive_index + i]]);
            for (uint32_t axis = 0; axis < 3; axis++) {
                ...
                for (uint32_t i = 0; i < node.primitive_count; i++) {
                    uint32_t bin_id = std::min(options.o_samples - 1, static_cast<uint32_t>((centers[bvh.primitive_indices[node.as.leaf.first_primitive_index + i]][axis] - split_bounds.min[axis]) * scale));
                    bins[bin_id].primitive_count++;
                    bins[bin_id].aabb.grow(aabbs[bvh.primitive_indices[node.as.leaf.first_primitive_index + i]]);
                }
        ...
        default: throw std::runtime_error("selected object split search type not implemented!");
    }

    return best_split;
}

uint32_t partition_node_indices(bvh_t& bvh, uint32_t node_index, const split_t& split, vec3* centers) {
    node_t& node = bvh.nodes[node_index];
...
void try_split_node(bvh_t& bvh, uint32_t node_index, aabb_t *aabbs, vec3 *centers, const options_t& options);

bool split_node(bvh_t& bvh, uint32_t node_index, const split_t& split, aabb_t *aabbs, vec3 *centers, const options_t& options) {
    if (split.axis == std::numeric_limits<uint32_t>::max()) return false;

    node_t& node = bvh.nodes[node_index];

    uint32_t right_first_primitive_index = partition_node_indices(bvh, node_index, split, centers);

    if (node.as.leaf.first_primitive_index == right_first_primitive_index | node.as.leaf.first_primitive_index + node.primitive_count == right_first_primitive_index) return false;

    node_t left = new_node(node.as.leaf.first_primitive_index, right_first_primitive_index - node.as.leaf.first_primitive_index);
    node_t right = new_node(right_first_primitive_index, node.as.leaf.first_primitive_index + node.primitive_count - right_first_primitive_index);

    node.is_leaf = false;
    // TODO: change this to an assert
    if (number_of_bits_required_for_number(bvh.nodes.size()) > FIRST_INDEX_BITS_SIZE) {
        throw std::runtime_error("number doesnt fit in FIRST_INDEX_BIT_SIZE bits");
    }
...
void try_split_node(bvh_t& bvh, uint32_t node_index, aabb_t *aabbs, vec3 *centers, const options_t& options) {
    ...
    const split_t split = find_best_split(bvh, node_index, aabbs, centers, options);
