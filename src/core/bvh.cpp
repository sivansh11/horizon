...
node_t new_node(uint32_t first_primitive_index, uint32_t primitive_count) {
  node_t node{};
  horizon_assert(!node.aabb.is_valid(),
                 "node should be defaulted to not valid");
  node.is_leaf = true;
...
uint32_t partition_node_indices(bvh_t &bvh, uint32_t node_index,
                                const split_t &split, vec3 *centers) {
  node_t &node = bvh.nodes[node_index];
...
bool split_node(bvh_t &bvh, uint32_t node_index, const split_t &split,
                aabb_t *aabbs, vec3 *centers, const options_t &options) {
...
bvh_t build_bvh2(aabb_t *aabbs, vec3 *centers, uint32_t primitive_count,
                 const options_t &options) {
  bvh_t bvh{};
