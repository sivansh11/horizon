...
public: 
    ...
    void set(uint32_t n) {
        horizon_assert(n < (sizeof(storage_t) * 8), ".");
...
struct base_component_pool_t {
    ...
    void *get(entity_id_t index) {
        horizon_assert(index < _max_entities, "index is out of bounds");
        horizon_assert(_p_data, "component pool was not created ?");
...
public:
    ...
    entity_id_t create() {
        horizon_assert(_available_entities.size() > 0, "entity limit reached!");
        ...
        if (_entities.size() <= id) {
            ...
            horizon_assert(!_entities[id].is_valid, "new entity with id {} already in used, yet it is in the available entities stack", id);
        }
        return id;
    }

    void destroy(entity_id_t id) {
        horizon_assert(id != null_entity_id, "cant destroy null entity");
        horizon_assert(_entities[id].is_valid, "cant destroy a destroyed entity");
    ...
    T& get(entity_id_t id) {
        horizon_assert(id != null_entity_id, "cant get from null entity");
        horizon_assert(_entities[id].is_valid, "cant get from deleted entity");
    ...
    T& construct(entity_id_t id, args_t&&... args) {
        horizon_assert(id != null_entity_id, "cant get from null entity");
        horizon_assert(_entities[id].is_valid, "cant get from deleted entity");
        horizon_assert(!_entities[id].mask.test(get_component_id_for<T>()), "entity already has component");
    ...
    void remove(entity_id_t id) {
        horizon_assert(id != null_entity_id, "cant get from null entity");
        horizon_assert(_entities[id].is_valid, "cant get from deleted entity");
        horizon_assert(_entities[id].mask.test(get_component_id_for<T>()), "entity doesnt have component");
