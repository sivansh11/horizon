...
struct config_descriptor_set_layout_t {
  config_descriptor_set_layout_t &add_layout_binding(
      ...
      VkShaderStageFlags vk_shader_stages, uint32_t vk_count = 1);
...
struct config_pipeline_layout_t {
  config_pipeline_layout_t &add_descriptor_set_layout(
      handle_descriptor_set_layout_t handle);
  config_pipeline_layout_t &add_push_constant(
...
struct config_pipeline_t {
  ...
  config_pipeline_t &add_vertex_input_binding_description(
  ...
  config_pipeline_t &set_pipeline_input_assembly_state(
      const VkPipelineInputAssemblyStateCreateInfo
          &vk_pipeline_input_assembly_state);
  config_pipeline_t &set_pipeline_rasterization_state(
      const VkPipelineRasterizationStateCreateInfo
          &vk_pipeline_rasterization_state);
  config_pipeline_t &set_pipeline_multisample_state(
...
struct update_descriptor_set_t {
  update_descriptor_set_t &push_buffer_write(
...
class context_t {
 public:
  ...
  std::vector<handle_image_view_t> get_swapchain_image_views(
      handle_swapchain_t handle);
  std::optional<uint32_t> get_swapchain_next_image_index(
      handle_swapchain_t handle, handle_semaphore_t handle_swapchain,
  ...
  handle_descriptor_set_layout_t create_descriptor_set_layout(
      const config_descriptor_set_layout_t &config);
  void destroy_descriptor_set_layout(handle_descriptor_set_layout_t handle);
  internal::descriptor_set_layout_t &get_descriptor_set_layout(
      handle_descriptor_set_layout_t handle);

  handle_descriptor_set_t allocate_descriptor_set(
  ...
  internal::descriptor_set_t &get_descriptor_set(
      handle_descriptor_set_t handle);

  handle_pipeline_layout_t create_pipeline_layout(
      const config_pipeline_layout_t &config);
  void destroy_pipeline_layout(handle_pipeline_layout_t handle);
  internal::pipeline_layout_t &get_pipeline_layout(
  ...
  handle_command_pool_t create_command_pool(
  ...
  handle_commandbuffer_t allocate_commandbuffer(
