...
template <size_t MAX_FRAMES_IN_FLIGHT>
struct managed_timer_t {
  std::array<handle_timer_t, MAX_FRAMES_IN_FLIGHT> handle_timers;
  resource_update_policy_t                         update_policy;
};
...
template <size_t MAX_FRAMES_IN_FLIGHT>
struct update_managed_descriptor_set_t {
  update_managed_descriptor_set_t &push_buffer_write(
      uint32_t binding, const managed_buffer_descriptor_info_t &info,
      uint32_t array_element) {
    horizon_profile();
    internal::managed_descriptor_set_t<MAX_FRAMES_IN_FLIGHT>
  ...
  update_managed_descriptor_set_t &push_image_write(
