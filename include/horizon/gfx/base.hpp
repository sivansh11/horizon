...
struct managed_buffer_t {
    std::array<handle_buffer_t, MAX_FRAMES_IN_FLIGHT> handle_buffers;
    resource_update_policy_t update_policy;
};

template <size_t MAX_FRAMES_IN_FLIGHT>
struct managed_descriptor_set_t {
    std::array<handle_descriptor_set_t, MAX_FRAMES_IN_FLIGHT> handle_descriptor_sets;
    resource_update_policy_t update_policy;
};
...
struct base_config_t {
    ...
    handle_image_view_t final_image_view;
};

template <size_t MAX_FRAMES_IN_FLIGHT>
struct update_managed_descriptor_set_t;
...
struct base_t {
    ...
    handle_managed_buffer_t create_buffer(resource_update_policy_t update_policy, const config_buffer_t& config);
    handle_buffer_t buffer(handle_managed_buffer_t handle);

    handle_managed_descriptor_set_t allocate_descriptor_set(resource_update_policy_t update_policy, const config_descriptor_set_t& config);
    handle_descriptor_set_t descriptor_set(handle_managed_descriptor_set_t handle);
    update_managed_descriptor_set_t<MAX_FRAMES_IN_FLIGHT> update_managed_descriptor_set(handle_managed_descriptor_set_t handle);
    ...
    std::map<handle_managed_buffer_t, internal::managed_buffer_t<MAX_FRAMES_IN_FLIGHT>> _buffers;
    std::map<handle_managed_descriptor_set_t, internal::managed_descriptor_set_t<MAX_FRAMES_IN_FLIGHT>> _descriptor_sets;
...
template <size_t MAX_FRAMES_IN_FLIGHT>
struct update_managed_descriptor_set_t {
    update_managed_descriptor_set_t& push_buffer_write(uint32_t binding, const managed_buffer_descriptor_info_t& info, uint32_t array_element) {
        horizon_profile();
        internal::managed_descriptor_set_t<MAX_FRAMES_IN_FLIGHT>& managed_descriptor_set = utils::assert_and_get_data<internal::managed_descriptor_set_t<MAX_FRAMES_IN_FLIGHT>>(handle, base._descriptor_sets);
        internal::managed_buffer_t<MAX_FRAMES_IN_FLIGHT>& managed_buffer = utils::assert_and_get_data<internal::buffer_t>(handle, base._buffers);
        if (managed_descriptor_set.update_policy == resource_update_policy_t::e_sparse) {
            check(managed_buffer.update_policy == resource_update_policy_t::e_sparse, "A sparse descriptor must only point to a sparse buffer!");
        }
    ...
    update_managed_descriptor_set_t& push_image_write(uint32_t binding, const image_descriptor_info_t& info, uint32_t array_element = 0) {
        horizon_profile();
        managed_descriptor_info_t managed_descriptor_info{};
        managed_descriptor_info.binding = binding;
        managed_descriptor_info.array_element = array_element;
    ...
    void commit() {
        ...
        internal::managed_descriptor_set_t<MAX_FRAMES_IN_FLIGHT> managed_descriptor_set = utils::assert_and_get_data<internal::managed_descriptor_set_t<MAX_FRAMES_IN_FLIGHT>>(handle, base._descriptor_sets);
        if (managed_descriptor_set.update_policy == resource_update_policy_t::e_sparse) {
            auto update_descriptor_set = base._info.context.update_descriptor_set(managed_descriptor_set.handle_descriptor_sets[0]);
            for (auto& info : infos) {
                if (info.type == managed_descriptor_info_type_t::e_buffer) {
                    internal::managed_buffer_t<MAX_FRAMES_IN_FLIGHT>& buffer = utils::assert_and_get_data<internal::managed_buffer_t<MAX_FRAMES_IN_FLIGHT>>(info.as.buffer_info.handle, base._buffers);
            ...
            for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
                auto update_descriptor_set = base._info.context.update_descriptor_set(managed_descriptor_set.handle_descriptor_sets[i]);
                for (auto& info : infos) {
                    if (info.type == managed_descriptor_info_type_t::e_buffer) {
                        internal::managed_buffer_t<MAX_FRAMES_IN_FLIGHT>& managed_buffer = utils::assert_and_get_data<internal::managed_buffer_t<MAX_FRAMES_IN_FLIGHT>>(info.as.buffer_info.handle, base._buffers);
