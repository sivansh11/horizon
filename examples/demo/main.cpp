...
viewport_t create_viewport(core::ref<gfx::base_t> base,   //
  ...
  ci.debug_name                  = "image";
  gfx::handle_image_t      image = base->create_image(ci);
  gfx::handle_image_view_t image_view = base->create_image_view(
  ...
  ci.debug_name                  = "depth";
  gfx::handle_image_t      depth = base->create_image(ci);
  gfx::handle_image_view_t depth_view = base->create_image_view(
...
void destroy_viewport(core::ref<gfx::base_t> base, const viewport_t &viewport) {
  base->wait_idle();
  base->destroy_image(viewport.image);
  base->destroy_image_view(viewport.image_view);
  base->destroy_image(viewport.depth);
  base->destroy_image_view(viewport.depth_view);
}
...
gfx::handle_image_t create_image_from_color(core::ref<gfx::base_t> base,
  ...
  gfx::handle_commandbuffer_t cmd =
      gfx::helper::begin_single_use_commandbuffer(*base);
  ...
  gfx::helper::end_single_use_command_buffer(*base, cmd);
  return image;
}
...
int main(int argc, char **argv) {
  ...
  gfx::handle_sampler_t          sampler   = base->create_sampler({});
  ...
  gfx::handle_descriptor_set_layout_t imgui_dsl =
      gfx::helper::create_imgui_descriptor_set_layout(*base);

  gfx::handle_descriptor_set_t imgui_ds = base->allocate_descriptor_set(
  ...
  base->update_descriptor_set(imgui_ds)
      .push_image_write(
  ...
  for (auto &raw_mesh : raw_model.meshes) {
    ...
    mesh.vertices =
        gfx::helper::create_buffer_staged(*base,                     //
                                          cb,                        //
    ...
    if (it != raw_mesh.material_description.texture_infos.end()) {
      gfx::handle_image_t image =
          gfx::helper::load_image_from_path_instant(*base,          //
                                                    it->file_path,  //
                                                    VK_FORMAT_R8G8B8A8_SRGB);
      gfx::handle_image_view_t image_view =
          base->create_image_view({.handle_image = image});
  ...
  // layout
  gfx::handle_pipeline_layout_t pl = base->create_pipeline_layout(cpl);
  gfx::config_pipeline_t        cp{};
  cp.handle_pipeline_layout = pl;
  cp.add_color_attachment(gfx::helper::image_format(*base, viewport.image),
                          gfx::default_color_blend_attachment());
  cp.set_depth_attachment(
      gfx::helper::image_format(*base, viewport.depth),
  ...
  cp.add_shader(gfx::helper::create_slang_shader(  //
      *base, "./examples/demo/shaders/diffuse.slang",
      gfx::shader_type_t::e_vertex));
  cp.add_shader(gfx::helper::create_slang_shader(
      *base, "./examples/demo/shaders/diffuse.slang",
      gfx::shader_type_t::e_fragment));
  gfx::handle_pipeline_t p = base->create_graphics_pipeline(cp);
  ...
  while (!window->should_close()) {
    ...
    rendergraph
        .add_pass([&](gfx::handle_commandbuffer_t cmd) {
          ...
          base->cmd_bind_descriptor_sets(cmd, p, 0,
                                         {base->_bindless_descriptor_set});
          base->cmd_set_viewport_and_scissor(cmd,          //
                                             vk_viewport,  //
                                             vk_scissor);
          for (auto &mesh : meshes) {
            ...
            pc.vertices = gfx::to<model::vertex_t *>(
                base->get_buffer_device_address(mesh.vertices));
            pc.indices = gfx::to<uint32_t *>(
                base->get_buffer_device_address(mesh.indices));
            pc.b_diffuse = mesh.diffuse;
            pc.b_sampler = b_sampler;
            base->cmd_push_constants(cmd,                      //
                                     p,                        //
                                     VK_SHADER_STAGE_ALL,      //
                                     0,                        //
