...
int main(int argc, char **argv) {
	...
	cpl.add_push_constant(2 * sizeof(VkDeviceAddress), VK_SHADER_STAGE_COMPUTE_BIT);
	...
	gfx::handle_commandbuffer_t cbuf = gfx::helper::begin_single_use_commandbuffer(ctx, command_pool);
	ctx.cmd_bind_pipeline(cbuf, p);
	ctx.cmd_push_constants(cbuf, p, VK_SHADER_STAGE_COMPUTE_BIT, 0, 2 * sizeof(VkDeviceAddress), pc);
