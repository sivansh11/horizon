...
struct shade {
    shade(gfx::base_t& base) : base(base) {
        ...
        cdsl.add_layout_binding(0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_ALL);
        ...
        ds = base._info.context.allocate_descriptor_set({ .handle_descriptor_set_layout = dsl });
    ...
    void update_descriptor_set(gfx::handle_image_view_t image_view) {
        base._info.context.update_descriptor_set(ds)
            .push_image_write(0, { .handle_sampler = gfx::null_handle, .handle_image_view = image_view, .vk_image_layout = VK_IMAGE_LAYOUT_GENERAL })
            .commit();
    }
